...
local function getWifidogStatus()
...
local function setTabPolicyOptions(option, policy)
	option.size = 200 .. '" style="width: 50%'
	option.rows = 8
	option.rmempty = true
	option.cfgvalue = function(self, section)
		local domain = self.map:get(section, policy) or ""
		return domain:gsub(",", "\r\n")
	end
	option.write = function(self, section, value)
		if not value then value = "" end		
		return self.map:set(section, policy, value:gsub("\r\n", ","))
	end
end
...
s = m:section(TypedSection, "wifidog", translate("运行状态"), getWifidogStatus())
s.anonymous = true
...
-- 访问控制
if luci.sys.call("dnsmasq -v | grep -q no-ipset") ~= 0 then
	o = s:taboption("policy", TextValue, "trusted_pan_domains", translate("泛域名白名单"), translate("不用通过认证既可访问的网址及其所有子域名,例: qq.com  多个域名请用回车隔开"))
	setTabPolicyOptions(o, "trusted_pan_domains")
end

o = s:taboption("policy", TextValue, "trusted_domains", translate("域名白名单"), translate("不用通过认证既可访问的网址，不含\"http://\", 多个URL请用回车隔开"))
setTabPolicyOptions(o, "trusted_domains")

o = s:taboption("policy", TextValue, "trusted_iplist", translate("IP白名单"), translate("不用通过认证既可连接的ip, 多个地址请用回车号隔开"))
setTabPolicyOptions(o, "trusted_iplist")

o = s:taboption("policy", TextValue, "trusted_maclist", translate("MAC白名单"), translate("非强制认证的用户MAC，多个MAC请用回车号隔开"))
setTabPolicyOptions(o, "trusted_maclist")

o = s:taboption("policy", TextValue, "untrusted_maclist", translate("MAC黑名单"), translate("禁止访问网络的MAC，多个MAC请用回车号隔开"))
setTabPolicyOptions(o, "untrusted_maclist")
